...
def handle_swipe():
  ...
  try:
    ...
    swipes[item_id][user_id] = direction # Store swipe on backend
...
# --- NEW ENDPOINT: Get Swipes for a User ---
@app.route('/swipes/<user_id>', methods=['GET'])
def get_user_swipes(user_id):
    """Returns a dictionary of item_id:direction for the given user."""
    if user_id not in ["Amitha", "Harsha"]:
        return jsonify({"error": "Invalid user_id specified."}), 400

    user_swipe_history = {}
    # Iterate through all items that have swipes recorded
    for item_id, item_swipes_dict in swipes.items():
        # Check if the requested user has swiped on this item
        if user_id in item_swipes_dict:
            user_swipe_history[item_id] = item_swipes_dict[user_id]
...
# --- NEW ENDPOINT: Delete Swipes for a User ---
@app.route('/swipes/<user_id>', methods=['DELETE'])
def delete_user_swipes(user_id):
    """Deletes all swipe records for the specified user."""
    global swipes # We need to modify the global swipes dictionary
    if user_id not in ["Amitha", "Harsha"]:
        return jsonify({"error": "Invalid user_id specified."}), 400

    cleared_count = 0
    # Need to iterate carefully as we modify the dictionary
    # Iterate through a copy of item IDs to avoid modification issues during iteration
    item_ids_to_check = list(swipes.keys())
    for item_id in item_ids_to_check:
        if item_id in swipes and user_id in swipes[item_id]:
            # Remove the user's swipe for this item
            del swipes[item_id][user_id]
            cleared_count += 1
            # Optional: If no users left for this item, remove the item entry?
            # if not swipes[item_id]:
            #     del swipes[item_id]

    # Note: This does NOT clear matches automatically. Resetting swipes
    # might logically imply matches should also be re-evaluated or cleared,
    # but the current logic keeps the matches list separate.
    # To clear matches too, uncomment:
    # global matches
    # matches = [] # Or selectively remove matches involving this user's swipes? More complex.

    print(f"Cleared {cleared_count} swipe records for user {user_id}")
    return jsonify({"message": f"Cleared {cleared_count} swipe records for user {user_id}."}), 200
